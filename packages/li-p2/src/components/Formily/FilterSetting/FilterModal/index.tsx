...
import type { FilterSettingItem, OptionType } from '../type';
...
export interface FilterModalProps {
   ...
   * 选择发生改变时
   */
  onChange: (value: FilterSettingItem[]) => void;
}
...
const FilterModal: React.FC<FilterModalProps> = (props) => {
  ...
  const [filterList, setFilterList] = useState<FilterSettingItem[]>([]);
  const [selectedFilterNode, setSelectedFilterNode] = useState<FilterSettingItem>();
  ...
  const onFilterChange = (val: FilterSettingItem) => {
  ...
  const addFilterItem = () => {
    const DEFAULTITEM = ({
      id: getUniqueId(),
      field: undefined,
    } as unknown) as FilterSettingItem;
        ...
        <div className={cls(`${prefixCls}__content__left`, hashId)}>
          ...
          <div>
            <DndProvider backend={HTML5Backend}>
              {filterList.map((item: FilterSettingItem, index: string | number) => (
                <div
