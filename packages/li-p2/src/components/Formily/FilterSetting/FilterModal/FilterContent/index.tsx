...
const FilterContent: React.FC<FilterContentProps> = (props) => {
  ...
  const [filter, setFilter] = useState<FilterSettingItem>(defaultValue);
  ...
  // 配置项变更
  const onFilterValueChange = (value: FilterSettingItem) => {
    setFilter(value);
    onChange(value);
  };
  ...
  return wrapSSR(
    <div className={cls(`${prefixCls}`, hashId)}>
      ...
      {filter.type === 'number' && (
        <NumberItem value={filter} range={domain as [number, number]} onChange={onFilterValueChange} />
      )}
    </div>,
  );
};
