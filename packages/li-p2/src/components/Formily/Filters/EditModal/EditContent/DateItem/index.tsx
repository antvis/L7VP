...
const DateItem: React.FC<DateItemProps> = (props) => {
  ...
  const { value: defaultValue, options: defaultOptions, onChange } = props;
  const timer: [string, string] | string = defaultValue.value;
  ...
  const onDateTypeChange = (e: 'datePicker' | 'rangePicker') => {
    setDateType(e);
    const _times = getTimeFormat(defaultValue.value[0], granularity?.value);
    if (_times) {
  ...
  const onGranularityChange = (e: string) => {
    ...
    if (_granularity) {
      const _value = {
        ...
        value: getTimeFormat(defaultValue.value, _granularity?.value),
      };
      // @ts-ignore
      onChange(_value);
      setGranularity(_granularity);
