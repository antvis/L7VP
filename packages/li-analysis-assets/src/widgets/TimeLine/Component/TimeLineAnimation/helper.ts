...
export const getTimeInterval = (
  ...
  const { startKey, endKey, steps: _steps } = getKeySteps(timeLine, selection);
  const lastKey = timeLine.length - 1;

  // 到达尾部
  if (endKey === lastKey) {
    const start = timeWindowType === 'moveTime' ? timeLine[0] : timeLine[startKey];
    const end = timeWindowType === 'moveTime' ? timeLine[_steps] : timeLine[startKey + 1];
    return [start, end];
  }
